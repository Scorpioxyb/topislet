...
@Test("无命令后 fresh 样本时结果不确定且不得 AX")
func noFreshSampleIsInconclusiveAndDoesNotFallback() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .playbackState(
            baselineTrackIdentity: "song-a",
            baselineIsPlaying: true,
...
@Test("只有一个不同 fresh 样本或重复 sampleID 时不得 AX")
func oneFreshUnchangedSampleIsInconclusive() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .playbackState(
            baselineTrackIdentity: "song-a",
            baselineIsPlaying: true,
        ...
        baselineSampleID: 10,
        observations: [
            .init(sampleID: 11, trackIdentity: "song-a", isPlaying: true),
            .init(sampleID: 11, trackIdentity: "song-a", isPlaying: true)
        ]
    )
...
@Test("unknown 或 PID 异常样本使确认保持不确定且不得 AX")
func invalidFreshSamplePreventsAXFallback() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .trackChange(baselineIdentity: "song-a"),
        baselineSampleID: 10,
        observations: [
            .init(
                sampleID: 11,
                trackIdentity: nil,
                isPlaying: nil,
                isValidQishuiSample: false
            ),
            .init(sampleID: 12, trackIdentity: "song-a", isPlaying: true),
            .init(sampleID: 13, trackIdentity: "song-a", isPlaying: true)
        ]
    )
...
@Test("两个不同 fresh 样本明确未变化时才允许一次 AX")
func twoFreshUnchangedSamplesAllowAXFallback() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .playbackState(
            baselineTrackIdentity: "song-a",
            baselineIsPlaying: true,
    ...
    #expect(result == .explicitlyUnchanged)
    #expect(PlaybackControlTimeline.shouldRunAXFallback(
        controlGeneration: 12,
        currentGeneration: 12,
        targetedDispatched: true,
...
@Test("目标播放态出现后立即确认且不得 AX")
func targetPlaybackChangeConfirmsWithoutAX() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .playbackState(
            baselineTrackIdentity: "song-a",
            baselineIsPlaying: true,
        ...
        baselineSampleID: 10,
        observations: [
            .init(sampleID: 11, trackIdentity: "song-a", isPlaying: false)
        ]
    )
...
@Test("目标曲目身份变化后立即确认且不得 AX")
func targetTrackChangeConfirmsWithoutAX() {
    let result = PlaybackControlTimeline.targetedControlConfirmation(
        expectation: .trackChange(baselineIdentity: "song-a"),
        baselineSampleID: 10,
        observations: [
            .init(sampleID: 11, trackIdentity: "song-b", isPlaying: true)
        ]
    )
...
@Test("旧 generation 即使明确未变化也不得触发新操作 AX")
func staleGenerationCannotFallbackAfterExplicitUnchanged() {
    #expect(!PlaybackControlTimeline.shouldRunAXFallback(
