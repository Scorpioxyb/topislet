...
- Apple Music 实验适配可读取歌曲、专辑封面、播放状态与进度，并通过定向 Apple Event 控制播放、切歌和绝对进度；前台切换到汽水或 Apple Music 时岛立即跟随，离开音乐应用后再按真实播放状态自动选择。
...
- 汽水音乐是当前产品级主适配；Apple Music 仍是实验适配，已支持专辑封面，暂不提供歌词。
