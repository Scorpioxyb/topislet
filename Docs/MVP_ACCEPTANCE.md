...
| A5 | AX 数据不污染进度 | AX 数据补充允许 `progress=unavailable`，不得覆盖主进度 | `--qishui-status` |
| A6 | 播放控制安全 | 单独播放汽水时，播放/暂停/切歌命中汽水 | 手动操作 |
| A7 | 视频冲突安全 | 抖音/QuickTime/浏览器视频播放时，不得误控视频 | 手动操作 |
| A8 | 汽水语义 AX 直控安全 | 其他媒体抢焦点时仍只控制汽水；前台 App 不变、鼠标不动、视频零误控 | 手动操作 |
| A9 | 点击交互稳定 | 连续点击展开/收起不跳位、不归零、不明显卡顿 | 手动操作 |
| A10 | 设置可用 | 设置、校准、状态诊断入口可打开且不自动弹窗 | 手动操作 |
| A11 | 单一主活动 | 正常启动和展开时不出现音乐、计时器、提醒三个固定标签 | 手动操作 |
| A12 | 临时提醒恢复 | 普通提醒约 3 秒后恢复此前模式和汽水最新状态；展开或拖动期间不抢占 | 手动操作 |
