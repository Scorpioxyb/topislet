...
- 展开封面右下角显示当前音乐来源的 18pt 真实 App 圆角方形图标，汽水对应 `com.soda.music`，角标向右下偏移 3pt 且不得被封面裁切；快速切换来源时不得短暂显示上一来源 Logo，找不到 App 时显示通用音乐符号，来源为空时不显示徽标。紧凑态 25pt 封面不得显示来源徽标。
- 上一首 / 下一首点击时外圈、图标位置和点击区均保持固定，只允许短促亮度变化与 0.96 按压缩放，不显示旋转等待环；快速连续点击仍按最后一次操作代次和 520ms 安全窗口处理，旧请求失败、超时或较早歌曲元数据回包不得影响最新操作。开启“减少动态效果”后不执行缩放或弹簧动画。
