...
- 汽水紧凑岛悬停展开时，顶部与主体只出现一套连续形变，不得出现窗口先下坠、内容再二次拉伸的弹窗感；移开后保持顶部居中并平滑恢复紧凑尺寸。
...
- 运行 `swift Scripts/verify-island-window-animation.swift`，确认单窗口、紧凑/展开目标尺寸、中心和顶边不漂移，并确认脚本结束后鼠标回到原位置。
