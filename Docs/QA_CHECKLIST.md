...
- 展开封面右下角显示当前音乐来源的真实 App 图标，汽水对应 `com.soda.music`；快速切换来源时不得短暂显示上一来源 Logo，找不到 App 时显示通用音乐符号，来源为空时不显示徽标。紧凑态 25pt 封面不得显示 22pt 来源徽标。
- 上一首 / 下一首每次点击立即产生轻微方向反馈，外圈和点击区保持固定；单次点击只允许同操作代次的歌曲变化完成反馈，快速连续点击从最后一次点击重新计算 180ms 等待门槛和 520ms 反馈窗口，期间旧请求失败、超时或较早歌曲元数据回包不得提前清除较新请求的等待态。开启“减少动态效果”后不执行位移、缩放或弹簧动画。
