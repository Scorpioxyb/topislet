...
- 点击播放/暂停、上一首、下一首时，若 macOS 当前媒体焦点确认在汽水音乐，应作用于汽水；若焦点被抖音/浏览器视频抢占，应阻断控制，不能误控视频播放器。
- 媒体焦点被视频占用时，拖动进度条不得控制视频播放器；当前版本可明确阻断，后续再验证定向 seek。

## 汽水专属控制

- 汽水音乐单独播放时，播放/暂停、上一首、下一首、进度拖动都命中汽水。
- 抖音/QuickTime/浏览器视频播放时，灵动岛控制不得误控视频；当前版本允许阻断控制并保留汽水显示。
- 汽水音乐未运行时，控制按钮不得误控当前系统媒体。
- 汽水窗口最小化或隐藏时，播放/暂停、上一首、下一首仍不能误控当前视频播放器；若无法确认汽水焦点，应给出受限状态。
